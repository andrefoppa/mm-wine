export type Wine = {
  id: number;
  name: string;
  producer: string;
  region: string;
  country: string;
  grape: string;
  vintage: number;
  price: number;
  priceRange: "ate50" | "ate100" | "ate150" | "acima150";
  category: "tinto" | "branco" | "espumante" | "rosé";
  momento: ("churrasco" | "romantico" | "presente" | "celebracao" | "diaadia" | "corporativo")[];
  badge?: string;
  score: number;
  notes: string[];
  enologoNote: string;
  image: string;
  body: number;
  tannins: number;
  acidity: number;
  finish: number;
  maluPick?: boolean;
};

export const wines: Wine[] = [
  {
    id: 1,
    name: "Quimera",
    producer: "Achaval Ferrer",
    region: "Mendoza",
    country: "Argentina",
    grape: "Malbec / Cabernet",
    vintage: 2021,
    price: 389,
    priceRange: "acima150",
    category: "tinto",
    momento: ["romantico", "presente", "corporativo"],
    badge: "Curadoria",
    score: 97,
    notes: ["Frutas negras", "Especiarias", "Tabaco", "Baunilha", "Chocolate amargo"],
    enologoNote:
      "Um blend extraordinário que equilibra potência e elegância. Taninos sedosos com longa persistência. Vinho de guarda excepcional.",
    image: "https://images.unsplash.com/photo-1474722883778-792e7990302f?w=600&q=90",
    body: 92,
    tannins: 80,
    acidity: 65,
    finish: 95,
    maluPick: true,
  },
  {
    id: 2,
    name: "Tignanello",
    producer: "Antinori",
    region: "Toscana",
    country: "Itália",
    grape: "Sangiovese / Cabernet",
    vintage: 2020,
    price: 890,
    priceRange: "acima150",
    category: "tinto",
    momento: ["celebracao", "presente", "corporativo"],
    badge: "Destaque",
    score: 98,
    notes: ["Cereja madura", "Couro", "Ervas finas", "Especiarias", "Terra"],
    enologoNote:
      "Ícone da Supertoscana. Complexidade ímpar, estrutura impecável. Um dos grandes vinhos do mundo disponível agora.",
    image: "https://images.unsplash.com/photo-1553361371-9b22f78e8b1d?w=600&q=90",
    body: 95,
    tannins: 88,
    acidity: 78,
    finish: 98,
    maluPick: true,
  },
  {
    id: 3,
    name: "Pouilly-Fuissé",
    producer: "Louis Jadot",
    region: "Borgonha",
    country: "França",
    grape: "Chardonnay",
    vintage: 2022,
    price: 310,
    priceRange: "acima150",
    category: "branco",
    momento: ["romantico", "celebracao"],
    badge: "Novo",
    score: 93,
    notes: ["Maçã verde", "Baunilha", "Avelã", "Mel", "Flores brancas"],
    enologoNote:
      "Chardonnay borgonhês clássico. Cremoso sem ser pesado, com mineralidade irresistível. Harmonia perfeita com frutos do mar.",
    image: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600&q=90",
    body: 70,
    tannins: 15,
    acidity: 85,
    finish: 88,
    maluPick: true,
  },
  {
    id: 4,
    name: "Gran Reserva Carménère",
    producer: "Concha y Toro",
    region: "Colchagua Valley",
    country: "Chile",
    grape: "Carménère",
    vintage: 2021,
    price: 129,
    priceRange: "ate150",
    category: "tinto",
    momento: ["churrasco", "diaadia"],
    score: 91,
    notes: ["Amora", "Pimenta verde", "Café", "Tabaco"],
    enologoNote:
      "A uva emblema do Chile na sua melhor expressão. Perfil único com pimenta verde característica e final persistente.",
    image: "https://images.unsplash.com/photo-1547595628-c61a29f498af?w=600&q=90",
    body: 82,
    tannins: 72,
    acidity: 70,
    finish: 85,
    maluPick: true,
  },
  {
    id: 5,
    name: "Cava Brut Nature",
    producer: "Gramona",
    region: "Penedès",
    country: "Espanha",
    grape: "Macabeo / Xarel·lo",
    vintage: 2020,
    price: 245,
    priceRange: "acima150",
    category: "espumante",
    momento: ["celebracao", "presente", "romantico"],
    badge: "Curadoria",
    score: 94,
    notes: ["Brioche", "Limão siciliano", "Maçã", "Mineral", "Amêndoa"],
    enologoNote:
      "Cava de altíssimo nível que rivaliza com Champagne. Borbulhas finas, autólise pronunciada. Aperitivo ou harmonização surpreendente.",
    image: "https://images.unsplash.com/photo-1510812431401-41d2bd2722f3?w=600&q=90",
    body: 55,
    tannins: 10,
    acidity: 90,
    finish: 82,
    maluPick: true,
  },
  {
    id: 6,
    name: "Whispering Angel Rosé",
    producer: "Château d'Esclans",
    region: "Provence",
    country: "França",
    grape: "Grenache / Rolle",
    vintage: 2023,
    price: 290,
    priceRange: "acima150",
    category: "rosé",
    momento: ["romantico", "celebracao", "presente"],
    badge: "Exclusivo",
    score: 92,
    notes: ["Pêssego", "Morango", "Flores", "Mineral", "Cítrico"],
    enologoNote:
      "O rosé mais famoso do mundo não decepcionou em 2023. Cor pálida salmão, boca delicada com frescor irresistível.",
    image: "https://images.unsplash.com/photo-1516594798947-e65505dbb29d?w=600&q=90",
    body: 52,
    tannins: 12,
    acidity: 82,
    finish: 80,
    maluPick: true,
  },
];
